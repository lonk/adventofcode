...
const coordinates = Object.keys(map).map(strCoordinates => strCoordinates.split(','));
...
for (cursorY = maxY; cursorY >= minY; cursorY--) {
    for (cursorX = minX; cursorX <= maxX; cursorX++) {
        if (map[`${cursorX},${cursorY}`] && map[`${cursorX},${cursorY}`] === 1) {
